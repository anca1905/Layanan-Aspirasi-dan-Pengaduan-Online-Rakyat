...
try {
    $status_filter = "status = 'Disetujui'";

    $stmt_total = $pdo->query("SELECT COUNT(*) FROM reports WHERE $status_filter");
    $total_laporan = $stmt_total->fetchColumn();

    $stmt_ringan = $pdo->query("SELECT COUNT(*) FROM reports WHERE severity = 'Ringan' AND $status_filter");
    $total_ringan = $stmt_ringan->fetchColumn();

    $stmt_sedang = $pdo->query("SELECT COUNT(*) FROM reports WHERE severity = 'Sedang' AND $status_filter");
    $total_sedang = $stmt_sedang->fetchColumn();

    $stmt_berat = $pdo->query("SELECT COUNT(*) FROM reports WHERE severity = 'Berat' AND $status_filter");
    $total_berat = $stmt_berat->fetchColumn();

    // Ambil 5 Laporan Terbaru
    $stmt_recent = $pdo->query("SELECT * FROM reports WHERE $status_filter ORDER BY created_at DESC LIMIT 5");
    $recent_reports = $stmt_recent->fetchAll(PDO::FETCH_ASSOC);

    // Ambil Laporan berdasarkan Kecamatan (dikelompokkan dengan desa)
    $stmt_kecamatan = $pdo->query("SELECT kecamatan, desa, COUNT(*) as total FROM reports WHERE $status_filter GROUP BY kecamatan, desa ORDER BY kecamatan ASC, total DESC");
...
$page_title = 'Beranda - Kabid Pemkab Bombana';
...
?>
        ...
        <div class="flex-grow-1">
            <div class="topbar d-flex justify-content-between align-items-center">
                <h4 class="mb-0 fw-bold" style="color: var(--primary-color);">Statistik Kerusakan</h4>
                <div class="d-flex align-items-center">
                    <span class="me-3 fw-bold">Halo, <?php echo $_SESSION['name'] ?? 'Kepala Bidang'; ?>!</span>
                    <img src="https://ui-avatars.com/api/?name=<?php echo urlencode($_SESSION['name'] ?? 'Kabid'); ?>&background=DAA520&color=fff" class="rounded-circle" width="40" alt="Admin">
                </div>
            </div>
                    ...
                    <div class="col-lg-8">
                        <div class="card border-0 shadow-sm mb-4">
                            <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center">
                                <h5 class="mb-0 fw-bold" style="color: var(--primary-color);"><i class="fas fa-list me-2"></i>5 Laporan Disetujui Terbaru</h5>
                                    ...
                                    <table class="table table-hover align-middle mb-0">
                                        ...
                                        <tbody>
                                            <?php if (empty($recent_reports)): ?>
                                                <tr>
                                                    <td colspan="5" class="text-center py-4">Belum ada laporan disetujui masuk.</td>
                                                </tr>
                                                <?php else: foreach ($recent_reports as $row): ?>
                                                    <tr>
                                                        ...
                                                        <td>
                                                            ...
                                                            if ($s == 'diproses') echo '<span class="badge bg-warning text-dark">Diproses</span>';
